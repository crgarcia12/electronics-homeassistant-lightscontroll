...
with open(sys.argv[2], 'w', newline='') as f:
    ...
    for group in net.groupComponents():
        ...
        for component in group:
            ...
            lcsc_pn = component.getField("JLCPCB") or lcsc_pn
            c = component

        if len(refs) == 0:
            continue
        name = (c.getValue() + " " + c.getDescription())[:64]
        name = name.replace(',', ' ').encode('ascii', 'ignore').decode()
        print(name)
        # Fill in the component groups common data
        out.writerow([name, ",".join(refs), c.getFootprint().split(':')[0], lcsc_pn])
